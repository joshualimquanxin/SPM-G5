...
AC2 Navigation and action controls outside a user's permitted set are not displayed. (frontend -
    see tests/e2e/rbac.spec.ts; the backend exposes the permission list on /auth/me for it)
...
# expected HTTP status per (role, action). 403 = signed in but not permitted.
MATRIX = {
    "EVENT_ORGANISER": {"list": 403, "create": 403, "update": 403},
    "EVENT_COORDINATOR": {"list": 200, "create": 403, "update": 403},
    "VENUE_STAFF": {"list": 200, "create": 201, "update": 200},
    "TECH_SUPPORT_STAFF": {"list": 200, "create": 403, "update": 403},
    "ATTENDEE": {"list": 403, "create": 403, "update": 403},
}
...
@pytest.mark.parametrize("action", ["list", "create", "update"])
@pytest.mark.parametrize("user", Users.ONE_PER_ROLE, ids=lambda u: u.role)
